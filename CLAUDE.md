...
*Version: v10.1 (compression refactor, Vic-authorized 2026-06-23: the pre-S129 inline precedent rosters were moved to `raw/notes/conventions-ledger.md` A.15–A.18 and prose was tightened — finishing the Section 5.3 forward-only migration. NO rule, convention, section number, frontmatter field, or analytical content changed.) Prior: v10.0 (tracker page type + `wiki/trackers/`; `layer` page type retired, the `layer` FIELD + Framework 2 placement unchanged; S154, 2026-06-12). Full codification history (v9.6→v10.0 and earlier) lives in the conventions ledger (A.1–A.14) + git.*
...
**Path discrepancy verification.** Verify path conventions before committing new vault paths. Skill dirs use hyphens (`.claude/skills/agent-onboarding/`); raw subdirs use no separators (`raw/filings/`). **`raw/filings/` + `raw/transcripts/` group sources into per-ticker uppercase-ticker subfolders** (`raw/filings/MP/MP-10K-2025-12-31.htm`; S123); `raw/filings/newly_fetched/` stays flat as the fetch/ingest staging queue. **The filename convention is the load-bearing invariant** (S126): every raw source is `TICKER-FORM-PERIOD.ext` (filings, e.g. `AVAV-10Q-2026-01-31.htm`) or `TICKER-earnings-DATE.ext` (transcripts, e.g. `AVAV-earnings-2026-03-10.pdf`). The ticker prefix is what *both* the per-ticker folder grouping *and* `fetch_filings.py`'s skip-check (ticker-derivation + recursive fallback) depend on, so naming must hold for every file — including Vic-staged transcripts. (The template `raw/transcripts/AXXX-earnings-YYYY-MM-DD.pdf` is an intentional reference artifact, not data — leave it.) Deeper nesting (per-year/quarter) is deferred under YAGNI — the filename's period already orders files within a ticker folder; revisit only if one ticker exceeds ~50 files. File locations: skill files → `.claude/skills/<skill-name>/SKILL.md`; reference docs → `raw/references/`; session prompts → `prompts/`; Vic-authored synthesis → `raw/notes/`. *See A.2.*
...
**Honest-verdict trigger discipline (codified Session 61).** When a refresh ingest surfaces evidence suggesting a baseline-reframing trigger may fire (Caveat #9 Layer 4-5 straddling upgrade; Outside Framework reframing; framework placement; A1 mode upgrade to reciprocal-confirmation), the trigger fires ONLY when evidence meets its pre-registered criteria — **NOT on directional momentum.** Assessing evidence honestly against the threshold and NOT firing despite suggestive evidence is the canonical methodology. Bidirectional: UPWARD reframing (kickoff hypothesis lower than primary evidence supports) follows the same threshold rigor. Cross-reference Section 3.5 A1 three-mode framing. *See A.10 for the precedent pattern; current count in MEMORY.md.*

**Mid-fiscal-year disclosure methodology shift pattern (codified Session 79; refined Session 86).** When a company restructures segments/divisions mid-fiscal-year OR retrospectively adjusts prior-period segment data, the canonical page must explicitly document the shift, apply restated comparatives at financial snapshot tables, and flag analytical-product complexity at cross-period comparison scope. Honest framing per Section 2.1: do not paper over the shift with blended-period framing; preserve segment-tier comparability boundaries at the shift date. *5-instance precedent (NVT/MOD/NOK/AMD/NVDA): See A.15.*

**Venue-asymmetric sub-pattern (codified Session 86; FIRST INSTANCE).** A shift may appear at one venue (earnings-call-presentation + investor website) while another preserves the prior framework (SEC 10-Q segment reporting). Document the venue-asymmetry at Source audit notes; cross-reference Section 3.6 cross-venue disclosure gap convention when it constitutes a cross-venue gap beyond mere reporting-venue lag. *FIRST INSTANCE (NVDA S81): See A.15.*
...
**Period-parity at calendar terms (primary discipline).** Compare cross-vault periods by calendar-quarter equivalence regardless of fiscal-year designation (e.g., FLNC Q2 FY2026 = calendar Q1 2026 — equivalent recency to Dec-31 peers' Q1 2026). Apply at Phase 0 source-set verification; document calendar-equivalence in Source audit notes per-source entry.

**Fiscal-year-designation honest framing (secondary discipline).** Preserve the fiscal-year designation at source filename + financial-table headers; annotate "Q2 FY2026 = calendar Q1 2026" in Thesis role at first cross-vault comparison.

**Applicability + LLM application.** Section 2.11 applies at **month+** fiscal-year-end offset scope; apply both disciplines at kickoff drafting + Phase 0 + Source audit notes per filer fiscal calendar. *7-instance month+ roster (5 distinct vault fiscal calendars: Mar 31 / late-June / Sep 30 / Oct 31, vs Dec-31 where 2.11 is N/A) + 3-instance week-scope baseline: See A.11.*

#### 2.11.1 Substantive-offset threshold methodology (codified Session 79; 3-instance week-scope precedent)

Not every non-calendar filer warrants Section 2.11. Threshold by fiscal-year-end offset from Dec 31: **week-scope (~3 days to ~1 month) = N/A** (treat as substantively-calendar — NVDA last-Sunday-of-January, AMD/INTC last-Saturday-of-December); **month+ (≥1 month) = Applied** (the 7-instance roster above). Honest-framing edge case: NVDA's ~1-month offset is larger than AMD + INTC's ~4-day offset but smaller than the month+ baseline, so NVDA is captured at the week-scope baseline; week-scope drift still yields a clean cross-canonical comparison. *Roster: See A.11.*

#### 2.11.2 Cross-canonical EXACT-period-parity comparative analytical product methodology (codified Session 79; FIRST INSTANCE)

When two vault canonicals share an EXACT-match fiscal calendar (identical period-end dates), direct cross-canonical comparison is enabled without offset reconciliation. LLM application: document the EXACT match in Thesis role + the cross-vault adjacency section. *FIRST INSTANCE (AMD S77 + INTC S78; FY2025 = Dec 27 2025, Q1 2026 = Mar 28 2026): See A.11.*
...
**Cross-reference scope.** Ties to Section 2.1 honest-verdict + Section 3.4 framing gap (distinct: cross-venue is venue-asymmetric, framing gap is tier-asymmetric) + the Section 2.1 mid-fiscal-year venue-asymmetric sub-pattern (codified Session 86). *5-reference precedent (NVDA S81 + NVDA-platform-integration S82 + cpo-integration S84 + CPO-platform-battle S84 + advanced-optical-packaging S85b): See A.16.*
...
**4-canonical sub-domain coverage strengthening (codified Session 86).** When a sub-domain accumulates 4+ canonical participants at one theme/chokepoint scope, Pathway 2 is reinforced but the 3-canonical threshold is unchanged — document with confidence at 3-canonical; 4-canonical is reinforcement, not a threshold-shift. *FIRST INSTANCE (AI-agentic-CPU-orchestration-reemergence: S80 ARM+AMD+INTC → S83 +NVDA Vera): See A.17.*

### 3.16 Language convention — "substantive" / "substantively" discipline (codified Session 86)

Default to direct language. Use "substantive" / "substantively" only where the word distinguishes Tier 1 primary substantiation from Tier 3 hedging or weak-fit assertion — marking something genuinely material vs marginal, not for emphasis. **Deletion test at composition:** each candidate asks "does this word add meaning here, or just emphasis?" — if emphasis, delete (pattern observed at S81; corrected S82 onward). If drift surfaces in a session output, surface it honestly in Phase 4 reflection; do not retroactively claim "load-bearing usage" for instances that fail the test.

**Application.** Prospective from Session 86; existing usage at NVDA.md S81 + log.md S81 + kickoff prompts preserved as historical artifact (forward-only per Section 3.8). Cross-reference Section 3.3 voice convention.
...
*5-instance Pathway precedent (S51 HALEU-fuel-chokepoint = FIRST 1-stop refresh; S65 TSMC-CoWoS = SECOND, substantial-content scope ~120 lines; S66 datacenter-laser-supply; S67 cpo-integration; S68 advanced-optical-packaging — S65–S68 = a 4-chokepoint paired-cluster refresh): See A.18.*
...
**6-K content variability.** 6-K filings span earnings, press releases, governance, material events — verify the selected 6-K is the earnings disclosure at Phase 0; the MJDS 6-K cover is wrapper-only (substantive content in three exhibits).

**Fiscal calendar + currency variability.** Foreign-issuer fiscal calendars may differ from US standard (Dec 31 most foreign incl. TSM/TSEM; Mar 31 many Japanese; late-Apr/late-May some Israeli/Indian) — verify period-end dates per A6 (h) at Phase 0. 20-F filings are typically USD with supplementary local-currency tables (clarify at Phase 0); apply frontmatter `foreign_issuer: true` per Section 3.2; `scripts/fetch_filings.py` may need extension for 20-F + 6-K + 40-F forms. *See A.8 for TSM precedent + multi-domain candidate list; S43 CCJ established the MJDS 6-file pattern.*
...
**Application + codification cadence.** Every Phase 0 verification runs all applicable pattern-type checks ((b)-(h) universal; (a) only when the kickoff pre-commits framework placement); gaps documented per Section 4.4. A6 codification refinement triggers at 5+ pattern types, revisited every codification session. Applies to kickoff drafting (chat-side) + Stop 1 Phase 0 verification (vault agent). *Per-instance evidence + cumulative counts in A.9 + MEMORY.md.*
...
**Application discipline + empirical ROI (codified Session 61).** Treat the boundary as binding: when uncertain whether a claim is primary-source-verified, default to "Structural context per Tier 3/4 sources to verify at primary." Stop 1 Phase 0 verification (Section 4.5 A6 (g')) tests primary-source content against the kickoff's "Codified background"; falsifications documented per Section 4.4. The four-tier discipline produced a multi-session zero-falsification streak: pre-codification (S45 ENS) = 6 falsifications; post-codification = 0/session (S47 FCEL = 1; subsequent sessions all 0, across the full ingest-type variety). Marketing-tier non-corroboration at primary is EXPECTED per the Section 4.5 discipline boundary, NOT a (g') variance. *See A.12 for the streak; current count in MEMORY.md; frameworks.md Section 11.11 for the cross-chokepoint theme framework.*

### 4.7 Refresh ingest log (`raw/notes/refresh_log.md`) (codified Session 104)

**Mandatory close-out step for every refresh ingest.** When a session re-ingests an EXISTING vault company page with a new primary source set (a "refresh ingest" per Section 4.1 / 4.2 — distinct from first-canonical creation, chokepoint/theme/relationship creation, codification, or in-place Tier-3 substrate refreshes), append a dated entry to `raw/notes/refresh_log.md` as part of the close-out, alongside `index.md` + `log.md` + MEMORY.md and before the final verification pass. `refresh_log.md` is a cumulative, reverse-chronological record of how each company's thesis evolves quarter over quarter (what changed since the prior baseline, why it mattered, placement changes, cross-vault propagation, forward watch items) — it complements `log.md` by isolating refresh-specific analytical deltas for longitudinal reference.

**Per-entry template** (newest at top; canonical format in the file header): `## S### — TICKER (Company) — YYYY-MM-DD — <period refreshed>`, then **Sources** / **Prior baseline** / **Headline** / **Key changes & new developments** / **Placement** (tier/layer change or "unchanged" + why) / **Cross-vault propagation** / **Forward watch** / **Key insight**. Keep entries to Section 3.8 brevity discipline. Not in `index.md`; updates do not count for accounting.
...
This file is expected to change. When a convention isn't working, flag it at end of session with a concrete proposal; the human decides and commits.

**Forward-only growth discipline (codified Session 129).** This always-loaded manual holds the **rules and current state**; the **empirical evidence, per-instance precedent rosters, canonical-example detail, and codification history** live in `raw/notes/conventions-ledger.md` (the codification analog of `refresh_log.md`). When a convention is codified, add the **rule** here and append its **supporting evidence / per-instance roster** to the ledger — never accumulate instance enumerations or precedent rosters in this file. Instance counts have a single live home (MEMORY.md per Section 3.8); the ledger holds the evidence behind them. The intent: this file does not grow monotonically. **Prospective** — existing rule text and inline calibrating examples are not retroactively trimmed.
