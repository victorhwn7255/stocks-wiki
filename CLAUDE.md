...
**Path discrepancy verification.** Verify path conventions before committing new vault paths. Skill directories use hyphens (`.claude/skills/agent-onboarding/`); raw subdirectories no separators (`raw/filings/`). **`raw/filings/` + `raw/transcripts/` group source files into per-ticker subfolders using uppercase tickers** (`raw/filings/MP/MP-10K-2025-12-31.htm`; codified Session 123); `raw/filings/newly_fetched/` stays flat as the fetch/ingest staging queue, and `scripts/fetch_filings.py`'s skip-check derives the ticker from the filename prefix. **The filename convention is the load-bearing invariant** (codified Session 126): every raw source is named `TICKER-FORM-PERIOD.ext` (filings — e.g. `AVAV-10Q-2026-01-31.htm`) or `TICKER-earnings-DATE.ext` (transcripts — e.g. `AVAV-earnings-2026-03-10.pdf`). The ticker prefix is what *both* the per-ticker folder grouping *and* the `fetch_filings.py` skip-check (ticker-derivation + recursive fallback) depend on, so the naming must hold for every file — including Vic-staged transcripts. A copy-paste naming template `raw/transcripts/AXXX-earnings-YYYY-MM-DD.pdf` is an intentional reference artifact (not data) — leave it in place. Deeper nesting (per-year/quarter subfolders) is deliberately deferred (YAGNI — the filename's period already orders files within a ticker folder); revisit only if a single ticker exceeds ~50 files. File locations: skill files → `.claude/skills/<skill-name>/SKILL.md`; reference docs → `raw/references/`; session prompts → `prompts/`; Vic-authored synthesis → `raw/notes/`. *See A.2.*
...
**Forward-only growth discipline (codified Session 129).** This file is the always-loaded operating manual: it holds the **rules and current state**. The **empirical evidence, per-instance precedent rosters, canonical-example detail, and codification history** that back those rules live in `raw/notes/conventions-ledger.md` (the codification analog of `refresh_log.md`). When a new convention is codified, add the **rule** here and append its **supporting evidence / per-instance roster** to the ledger — do not accumulate instance enumerations or precedent rosters in this file. Instance counts have a single live home (MEMORY.md per Section 3.8); the ledger holds the per-instance evidence behind them. The intent is that this file does not grow monotonically. This discipline is **prospective** — existing rule text and inline calibrating examples are not retroactively trimmed.
