...
> **Status (v0.2 — LIVING, the "keep-growing-and-optimizing" version; operative, not locked):** Working analytical framework for the vault's **third** thesis domain (humanoid robots), parallel to the AI-datacenter `frameworks.md` (v10.1) and the Defense & Drones `frameworks-defense-drones.md` (v1.0). Humanoid-first scope per `wiki/_thesis-humanoid-robot.md`. Promoted from v0.1 DRAFT on 2026-06-07 via a **Vic-authorized rework** (investor-access-lens reframe + ingest-progress refresh through Session 145); default human-ownership resumes after (LLM proposes at codification sessions, never silently edits). Created from two Tier-3 anchor reports (`raw/research/humanoid-robot-primer-report.md` + `humanoid-robot-high-value-subsystems-research.md`), anchored on Goldman Sachs' humanoid value-chain work. **Expected to keep evolving as primary-source evidence accumulates and the physical-AI broadening proceeds.**
...
A value-capture gradient for humanoid exposure quality. **Deliberately NOT codified as a frontmatter field yet (YAGNI):** even with ~9 humanoid names now ingested (S136→S145), a dedicated `humanoid_tier` field stays premature — names land **`outside` the existing per-domain frameworks + theme-anchored** (the [[INOD]] precedent), or carry **another domain's tier where cross-thesis** (e.g. [[HARMONIC]] `equipment_tier 4` + `defense_tier 4`; [[TUOPU]] `energy_power_tier 4`; [[NOVT]]/[[VPG]] `equipment_tier 4`), with the **access-route flag** carried in prose. A `humanoid_tier` (or, more likely, a broader `physical_ai_tier`) is a **human-owned codification** to revisit only when the name count and the physical-AI broadening justify it (now a closer call — flagged in the thesis anchor).

The gradient (highest to lowest durable value capture). The **"strongest chokepoints, worst *US* access" tension** appears in the US-access column below — but read those flags as **access *routes*, not barriers** (see the investor-access-lens note under the table):
...
🟢 US-listed clean / 🟡 foreign-listed (Japan/Germany/Taiwan/Australia) / 🔴 China A-share / HK or private — **access *routes*, not barriers:** 🔴 means "a China-access route is required" (Stock Connect / China-enabled broker), not "cannot own."

**Honest-verdict note:** the tier *letter* is not a ranking of conviction — Tier A (materials) carries the *highest* structural conviction. Conviction tracks the **chokepoint quality gradient (H5) + business health**, not the position in the build or the listing flag. The vault's edge here is surfacing what the banks' component-rankings underweight: the materials substrate (MP) and the high-end sensor layer (NOVT/VPG).

**Investor-access lens (the operative one for this vault).** The "strongest chokepoints / worst access" tension is a **general-US-book lens** — it assumes the reader cannot cleanly hold a China A-share. **Vic can** (US, foreign-listed, *and* China A-share / HK), so for this vault the tension **inverts**: the strongest mechatronic chokepoints (precision reducers, actuator assembly — Tier B) become the *most* ownable layer, not the least, and the operative ranking is the **chokepoint-quality gradient + business health, not the listing flag**. Keep the US-access column for completeness — a clean *US* line genuinely is missing at the top value layers, which matters for a US-only reader and is a real "missing pure-play" signal — but rank on durability + health. *(Describe-don't-recommend holds; the intended 5–10% China sleeve lives in operational notes, never on a vault page.)*
...
The highest-conviction lens, and the humanoid analogue of the Defense D5 (geology-over-policy) rule. **The central rule: rank chokepoints by what makes them durable. Physics/geology > precision-manufacturing know-how > assembly/integration — that durability gradient + business health is the operative ranking.** The **US-listed-access column** is a secondary, *reader-dependent* overlay: the "strongest chokepoints have the worst *US* access" tension is real for a US-only book (and a missing clean-US pure-play is a genuine signal), but for a China-access investor it **inverts** (the top chokepoints are directly ownable) — so treat access as a **route flag, not a ranking input** (see the H2 investor-access lens).
...
| **[[HARMONIC]] (6324.T)** | Reducers (B) | High (precision) | 🟡 Japan | GS's "Buy" — gold-standard reducer; **booked AI-robot revenue** (the cohort's only confirmed-at-primary humanoid revenue) | **Ingested S145** (vault's first Japanese filer) — `equipment_tier 4` + `defense_tier 4` + theme-anchored humanoid; fortress balance sheet at a cyclical trough; reachable via Japan |
...
| **[[SANHUA]] / [[TUOPU]] / [[SHUANGHUAN]] / [[ZHONGDA]]** (+ LeaderDrive/绿的谐波, Inovance, Zhaowei, Hengli, CATL, Sunny, RoboSense, Hesai) | Assembly / reducers / hands / motors / vision / battery | mixed | 🔴 China A/HK (reachable via China-access route) | China-dominated layers | **The four named cohort are now full vault pages (S141–S144)**; the rest map-only pending ingest — China-access route required, not "un-investable" |

**Private / un-investable (mark and monitor):** Figure AI ($39bn), 1X, Agility, Apptronik, Sanctuary, Maxon, DJI/Livox, Unitree (pre-STAR-IPO), UBTech (HK-listed). **This is now the *real* un-investable set** (the China A-share names are reachable via a China-access route) — and, as with Defense, the best pure-play capability is often not publicly investable at all: a structural feature, not a temporary gap.

**Deep-dives completed (S136→S145):** [[MP]] → [[NOVT]] / [[VPG]] → [[OUST]] → the actuator/reducer cohort [[SANHUA]] / [[TUOPU]] / [[SHUANGHUAN]] / [[ZHONGDA]] → [[HARMONIC]] (the GS-Buy reducer gold standard, the vault's first Japanese filer). **Next:** the remaining harmonic / roller-screw names (LeaderDrive/绿的谐波, Schaeffler, Nabtesco) via a consolidated chokepoint page; then the physical-AI broadening. **NVDA** carries a light humanoid-optionality note only.

---

## How the LLM should use these frameworks

1. **For first-pass categorization:** place each humanoid company in its value-chain position (H1), its value-capture tier letter (H2), and its chokepoint position + durability (H5). **No `humanoid_tier` frontmatter field** — names land `outside` the existing frameworks + theme-anchored ([[INOD]] precedent), or carry another domain's tier where cross-thesis (e.g. HARMONIC `equipment 4`+`defense 4`); the **access-route flag** lives in prose.

2. **Rank on chokepoint-quality + business health (H2/H5); treat access as a route, not a ranking input.** The "strongest chokepoints / worst *US* access" tension is a general-US-book lens that **inverts** for a China-access investor (the operative case for this vault) — so pair a chokepoint's durability with its *route* (US / foreign / China-access), not a binary "investable?" The vault's edge is surfacing the *edges the banks underweight* — the materials substrate (MP) and high-end sensors (NOVT/VPG) — plus the now-reachable top-layer chokepoints ([[HARMONIC]]; the China reducer/actuator cohort).
