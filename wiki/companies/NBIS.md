...
**Honest-verdict framing (Section 2.1).** Like CoreWeave, Nebius holds **no chokepoint** — it is a capacity provider and price-taker buying merchant NVIDIA GPUs. Its inclusion is **not** for thesis centrality; it is the **2nd rent-side data point** (making the neocloud fragility a pattern, not a one-off) and the **most extreme capex-vs-revenue case in the program.** Two honest nuances distinguish it from CoreWeave, though: its **core cloud segment is adjusted-EBITDA-positive** (below), and it carries a large **cash buffer** rather than CoreWeave's heavier net debt — so it is the more aggressive *spender* but, for now, the better-*funded* one. The cross-vault moat-durability verdict is at [[neocloud-moat-durability]] — where Nebius reads as the *more capital-structure-resilient, less circular* of the two pure-plays (cheaper convertible debt, no single-contract GPU-SPVs, larger cash buffer), though that head-to-head edge is partly inferential pending filing-grade Nebius concentration/leverage disclosure.
...
## Cross-vault research note (Tier-3 — 2026-06-29; verify at next primary refresh)

The S177 neocloud research-loop (`raw/research/interactive/2026-06-29_neocloud-moat-durability-crwv-vs-nbis.md`) dug the Q1 2026 6-K for the [[neocloud-moat-durability]] head-to-head and surfaced several items to fold in at the next primary NBIS refresh. **All Tier-3 pending own-read verification; canonical financials above + `last_updated` unchanged.**

- **Concentration (advances Open Q#3 — and a correction):** Microsoft (~$17.4–19.4B) + Meta together ≈ **>90% of backlog** — i.e. Nebius is **equally/more concentrated than CoreWeave, *not* less.** The cross-vault verdict's earlier "Nebius less concentrated" read has been corrected at [[neocloud-moat-durability]]; Nebius's real edge is the **balance sheet, not diversification.** (No customer %-split is disclosed; the >90% is an inference — confirm the >10%-customer note at the next 20-F.)
- **Capital structure (specifics behind the "better-funded" framing):** convertibles **~$4.34B** ($2.59B @1.25%/2031 + $1.75B @2.625%/2033) — low-coupon, long-dated, **no 2026–27 maturity wall**; deferred revenue **~$4.78B** (customer prepayment).
- **Earnings-quality flag (§4.3 — verify, don't overwrite):** the research read Q1 2026 **GAAP net income as positive ~$621M, flattered by a ~$780.6M one-time ClickHouse equity-revaluation gain → adjusted net loss ~$(100.3)M.** This sits *against* the vault's own 6-K read of a **$(113.5)M net loss** above — reconcile the exact net-income line (which includes/excludes the revaluation gain) at the next refresh.
- **Meta agreement scope (§4.3):** the research attributes the **$27B/5-yr** figure to the **Meta** deal; this page currently reads $15B as Meta and the $27B+ as a *separate, Microsoft-consistent* agreement — confirm which counterparty the $27B belongs to at the next 20-F.
- **NVIDIA instrument:** the 6-K summary loosely called the $2B a NVIDIA "equity investment"; the vault's **20-F line-item ("~$2B proceeds + pre-funded Class A warrant, ~21M shares") stands as primary** — confirm the equity-vs-warrant classification at the next 20-F (also flagged at [[nvidia-supply-chain-commitments]]).
...
3. **Microsoft/Meta concentration durability** — how exposed is the contracted demand to its two anchor customers? *(Tier-3 partial answer, 2026-06-29: MSFT+Meta ≈ **>90% of backlog** — high concentration; see the cross-vault research note above. Confirm the exact %-split at the next 20-F.)*
...
- **2026-06-29 (S177 — cross-vault research propagation; Tier-3, last_updated unchanged):** Added a "Cross-vault research note" from the neocloud research-loop — concentration (MSFT+Meta ≈ >90% of backlog → Nebius is *not* less concentrated than CoreWeave; the [[neocloud-moat-durability]] head-to-head was corrected accordingly), convertibles ~$4.34B / no 2026–27 wall, deferred revenue ~$4.78B, an earnings-quality flag (GAAP net income possibly +$621M flattered by a ~$780.6M ClickHouse gain vs this page's $(113.5)M net loss — to reconcile), and the Meta $27B-vs-$15B attribution question. No primary re-ingest; canonical financials + the 20-F "$2B + pre-funded warrant" unchanged; verify all at the next 20-F/6-K.
- **2026-06-16 (S164):** Added [[neocloud-moat-durability]] theme cross-ref (back-link) in Thesis role — the durability-verdict home, where NBIS reads as the more resilient of the two pure-plays (head-to-head partly inferential pending Nebius filing-grade disclosure). No financial content changed; last_updated unchanged.
