...
- **Black Hornet** nano-UAS — "Black Hornet 3, now Black Hornet 4, will have revenues of about **$500 million** over that period"; orders in the US, Europe, and the Middle East (Mehrabian). The Black Hornet's cells come from the battery enabler [[AMPX]] (Amprius names Teledyne FLIR as a customer; its SiCore cells are designed into FLIR UAS programs) — so this nano-drone is a node where Teledyne *consumes* the battery chokepoint even as it *owns* the sensor one (a second squeezed-middle dependency, alongside the rare-earth/germanium one below).
...
- **2026-06-04 (Session 133 — cross-ref):** Added to the new vault-wide [[china-exposure]] theme as a **squeezed-middle node** — owns the EO/IR sensor chokepoint yet consumes the China-controlled rare-earth-magnet + germanium/gallium chokepoint. No content change.

- **2026-06-04 (Session 132 — cross-ref):** Added a reciprocal link to [[AMPX]] (Amprius, now a vault page) at the Black Hornet bullet — Teledyne FLIR is a named Amprius battery customer, so the Black Hornet is a node where Teledyne *consumes* the battery chokepoint even as it *owns* the EO/IR sensor one. No other content change.
