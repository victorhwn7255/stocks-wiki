...
**Status:** **v0.2 — LIVING ANCHOR (the "keep-growing-and-optimizing" version; operative, but deliberately *not* locked/final).** Third thesis domain in the stocks-wiki vault, parallel to the AI-datacenter thesis (`wiki/_thesis.md`) and the Defense & Drones thesis (`wiki/_thesis-defense-drones.md`). Promoted from v0.1 DRAFT on 2026-06-07 via a **Vic-authorized rework** (per CLAUDE.md Section 1.1): investor-access-lens reframe + ingest-progress refresh through Session 145 — after which default human-ownership resumes (never edited by the LLM by default; refine only at future Vic-authorized passes). It is **expected to keep evolving** as primary-source evidence accumulates and the physical-AI broadening proceeds — that "keep-growing-and-optimizing" posture is the point, not a sign of incompleteness. Shares the vault's methodology, source hierarchy, and disciplines; does not replace or modify the other two anchors. **Scope: humanoid-first; broadens to "physical AI" / embodied AI later** (the same one-domain-at-a-time expansion mechanism the vault used to add Defense).
...
1. **Where are the durable opportunities in humanoids?** Across the brain (foundation models + compute), actuation (the mechatronic core), manipulation (hands), perception (sensors + vision), power (batteries), and the materials substrate — which positions are structurally durable versus commoditizing? **I can reach US-listed, foreign-listed (Japan/Germany/Taiwan), and China A-share / HK names** (incl. via Stock Connect / a China-enabled broker), so the operative ranking is the **chokepoint-quality gradient + business health, not the listing flag.** The old finding — *the strongest chokepoints have the worst US-listed access* — still holds as a **general-US-book lens** (a clean *US* line genuinely is missing at the top value layers), but for my own access it largely dissolves: the strongest mechatronic chokepoints (precision reducers, actuator assembly — mostly Chinese A-share or Japanese) are directly ownable, so I score them on durability and health, not on whether a US ticker exists. *(Allocation note lives off the wiki: a 5–10% China sleeve is intended, but no position-sizing ever appears on a vault page.)*
...
The demand thesis is credible on the **direction** of automation. It is weakest on **timing** (realistic near-term volumes are tiny; every Tesla/Figure timeline has slipped) and on **whether the makers commoditize the whole stack** — but **not on access**: because I can hold China A-shares, Japanese, and US names alike, the strongest mechatronic chokepoints (precision reducers, actuator assembly) are reachable, and the question becomes durability + business health, not listing venue. **The supply-side / chokepoint framing is the higher-conviction expression of the thesis.** Within it: the gold-standard reducer is Japanese ([[HARMONIC]] — Goldman's Buy); the deepest *US-listed* edges the banks underweight are the rare-earth-magnet substrate ([[MP]]) and the six-axis force/torque sensor layer ([[NOVT]] / [[VPG]]); and the Chinese actuator/reducer cohort ([[SANHUA]] / [[TUOPU]] / [[SHUANGHUAN]] / [[ZHONGDA]]) is directly ownable rather than map-only.
...
**Investability / quality tiers (a proposed `humanoid_tier`, deliberately NOT codified yet — held under YAGNI until the domain matures into a physical-AI codification):** rather than a tier field, names land **`outside` the existing per-domain frameworks + theme-anchored** (the [[INOD]] precedent), **or carry another domain's tier where they are cross-thesis** (e.g. [[HARMONIC]] = `equipment_tier 4` + `defense_tier 4`; [[TUOPU]] = `energy_power_tier 4`; [[NOVT]]/[[VPG]] = `equipment_tier 4`), with the **access-route flag (US-listed / foreign-listed / China-access)** carried in prose. See `frameworks-humanoid-robot.md` Framework H2.
...
**Harmonic / strain-wave reducers (high conviction; worst US access):** The precision gearbox at rotary joints (shoulder, elbow, wrist, neck) — flexspline + circular spline + wave generator giving huge reduction with near-zero backlash. Requires micron-precision grinding, specialized metallurgy, and limited high-precision grinding machines (a key Goldman bottleneck). Top-5 players (Harmonic Drive, Nabtesco, HDS, Wittenstein, Schaeffler) ~55–60% global share. Goldman's *top pick* (Exhibit 5 score 16). The gold-standard pure-play is Japanese — **[[HARMONIC]] (Harmonic Drive Systems, 6324.T), now ingested at primary (S145, the vault's first Japanese filer; Goldman's actual "Buy")** — reachable via Japan (🟡) though with no clean US ADR; the challengers are Chinese A-shares ([[SHUANGHUAN]] (RV), [[ZHONGDA]] (full-category incl. harmonic), LeaderDrive). A genuine gap remains specifically in the *US*-listed universe — with a commoditization clock as China localizes.
...
- **Cleanest US-listed chokepoint expressions (the edges the banks underweight):** **[[MP]]** (rare-earth magnets — triple-thesis; vault page), **[[NOVT]]** (Novanta — six-axis force/torque via ATI; vault page, S138), **[[VPG]]** (Vishay Precision — force/strain sensing; vault page, S139). *(Both sensor names: humanoid is small optionality on an AI-datacenter-equipment base — `equipment_tier 4`.)*
- **US-listed, already-owned or commoditizing:** **[[NVDA]]** (brain/compute — don't double-count; vault page), **TSLA** (the maker-with-a-brain), **RRX** (Regal Rexnord — cleanest US motion/motor), **[[INTC]]** (RealSense/edge — vault page), **[[OUST]]** / HSAI / INVZ / LAZR (LiDAR — higher-beta optionality; OUST a vault page, S140), AMBA / ST (watch).
- **Foreign-listed chokepoint pure-plays (reachable, not "awkward"):** **[[HARMONIC]]** (Harmonic Drive Systems, 6324.T, Japan — the GS-Buy reducer gold standard; **now a vault page, S145**, the vault's first Japanese filer), Schaeffler (SHA.DE, Germany — roller screws), HIWIN (2049.TW, Taiwan), Nidec (6594.T, Japan — motors), Lynas (LYC.AX, Australia — rare earths).
- **China A-share / HK (reachable via a China-access route — Stock Connect / China-enabled broker):** the actuator/reducer cohort **[[SANHUA]]** (A+H), **[[TUOPU]]** (A; H-IPO pending), **[[SHUANGHUAN]]** (A), **[[ZHONGDA]]** (A) are **all now full vault pages**; LeaderDrive/绿的谐波, Inovance, Zhaowei, Hengli Hydraulic, Leadshine, Moons' Electric, CATL, Sunny Optical, RoboSense, Hesai, Unitree (pre-IPO), UBTech remain map-only pending ingest.
- **Private / un-investable (mark and monitor):** Figure AI, 1X, Agility, Apptronik, Sanctuary, Maxon, DJI/Livox, Boston Dynamics (Hyundai sub). **This is now the *real* un-investable set** (the China names are reachable via a China-access route) — and, as with Defense, the best pure-play capability is often not publicly investable at all: a structural feature, not a temporary gap.

**Deep-dives completed (S136→S145):** [[MP]] (magnet substrate) → [[NOVT]] / [[VPG]] (US sensors) → [[OUST]] (LiDAR) → the actuator/reducer cohort [[SANHUA]] / [[TUOPU]] / [[SHUANGHUAN]] / [[ZHONGDA]] → [[HARMONIC]] (the GS-Buy reducer gold standard). **Next:** the remaining harmonic / roller-screw names (LeaderDrive/绿的谐波, Schaeffler, Nabtesco) via a consolidated chokepoint page; then the physical-AI broadening. **NVDA** carries a light humanoid-optionality note only (don't double-count).
...
- **The value stays private (no longer simply "in China").** With China A-shares reachable for me, "the chokepoints are Chinese" is no longer itself a falsifier — the live version is that the best capability stays **private** (Figure, 1X, Apptronik, Maxon) and never lists. (A *US-only* book still faces the original "value stays in China/private" falsifier in full; for my access only the private leg binds — the MP/NOVT US edges remain the partial answer either way.)
...
**The wiki is for:** synthesizing what I actually believe about humanoid value-chain dynamics; stress-testing each name's structural fit *and* its access route; surfacing chokepoints and cross-thesis nodes; tracking how positioning evolves; and generating uncomfortable conclusions (especially that the best chokepoints lack *clean US-listed* lines — reachable for me via foreign / China-access routes but awkward for a US-only book — and that "humanoid plays" are mostly optionality on legacy businesses).
...
**Note on this anchor (created 2026-06-05; reworked to v0.2 LIVING 2026-06-07):** Originally a **v0.1 DRAFT** drafted via Vic-authorized collaborative drafting (Vic-directed; agent-scaffolded) as the vault's third thesis domain, parallel to the AI-datacenter `_thesis.md` and the Defense & Drones `_thesis-defense-drones.md` (both untouched). **Promoted to v0.2 — the LIVING "keep-growing-and-optimizing" version (operative, not locked) — on 2026-06-07 via a Vic-authorized rework** (investor-access-lens reframe + ingest-progress refresh through Session 145); default human-ownership resumes after (LLM proposes, never silently edits; refine at future Vic-authorized passes). Sourced from two Tier-3 research reports (`raw/research/humanoid-robot-primer-report.md` + `humanoid-robot-high-value-subsystems-research.md`) anchored on Goldman Sachs' humanoid value-chain work (Exhibits 2 & 5), plus cross-bank corroboration (Morgan Stanley, JPM, BofA, McKinsey, Jefferies). **All figures are Tier-3 and forward TAM/unit/cost numbers ($38bn 2035 TAM, ~1.4M units, BOM splits, magnet kg/robot, China-concentration %s) are projections/estimates, not realized results — verify against primary filings (SEC 10-K/10-Q; company IR) before treating as load-bearing.** Scope is humanoid-first; the physical-AI broadening is sequenced for future expansion. Companion frameworks file: `raw/notes/frameworks-humanoid-robot.md` (reworked to v0.2 in the same pass). No `humanoid_tier` frontmatter field is codified (held under YAGNI; names land `outside` + theme-anchored per the [[INOD]] precedent, or carry another domain's tier where cross-thesis). The first theme under this anchor is `humanoid-robot-value-chain`.
