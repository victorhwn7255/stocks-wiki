...
| [[DDOG]] — x-ref (ingested S188) | 4 | Consumption (LLM spans) | LLM Obs spans ~tripled q/q, MCP tool calls 4×, Bits AI SRE investigations 2×+ (Q1 2026 call); **AI-native cohort now sized by growth-contribution — ~7pts of Q4 2025 YoY growth + "includes our largest customer" (FY2025 10-K)** — an advance from "declined to size," but still no clean cohort-revenue *level* | **Contested (feature-or-market) — trigger PARTIALLY met** | AI observability may not be separable; still bundled into span billing | Discloses AI-cohort revenue *level* >10% (would confirm separability) — advanced, not met |
...
- **AI observability may never be a separable market** — [[DDOG|Datadog]] bundles LLM Observability into span-based consumption billing; at primary (S188) it *advanced* from refusing to size the AI cohort to disclosing its growth contribution (~7pts of Q4 2025) + "includes our largest customer," but still no clean cohort-revenue *level* — so separability stays contested and standalone eval pricing stays structurally undercut.
...
- **2026-07-18 (S188 — [[DDOG]] first-canonical ingest; Layer-4 trigger partially met):** Updated the DDOG eval/observability row + the honest-counterpoint line from the primary ingest. The pre-registered trigger ("discloses AI cohort >10% of revenue") is now **partially met** — DDOG advanced from "declined to size the AI cohort" (Q4 2025) to disclosing the cohort's **growth contribution (~7pts of Q4 2025) + "includes our largest customer"** (FY2025 10-K) + count granularity (22 >$1M / 5 >$10M) — but still discloses **no clean cohort-revenue level**, so AI-observability **separability stays contested**. DDOG already in `tickers`. last_updated 2026-06-13 → 2026-07-18.
