...
**Why efficiency tricks don't dissolve it.** Quantization (FP16→FP8→FP4) is the honest partial exception — it cuts bytes-per-weight and genuinely eases bandwidth — but models expand to fill the freed memory. MoE/sparsity cut active-parameter *compute* yet the full experts must still sit in fetchable memory; speculative decoding helps only at the low batch sizes that are most bandwidth-bound. Net: efficiency attacks **compute and capacity**, not **bandwidth**, and bigger models / longer context / KV-cache re-saturate any headroom (a "memory-Parkinson" dynamic). This is why the demand-durability debate and the memory-bottleneck debate are *decoupled* (ties to [[AI-demand-durability]]).
