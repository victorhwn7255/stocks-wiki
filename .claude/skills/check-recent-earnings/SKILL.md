...
### 5. Auto-download the new filings into staging

If Section 5 of the report lists one or more new filings, run the download immediately after presenting the report:

```bash
python scripts/fetch_filings.py
```

This fetches every Section-5 filing into `raw/filings/newly_fetched/` (the staging/review queue). The fetcher's skip-if-present check across both `raw/filings/` and `newly_fetched/` guarantees it downloads exactly the Section-5 list and nothing else — no duplicates, no re-downloads. After the run, confirm to the user what was downloaded (ticker + form + filename) and remind that files sit in staging until a real ingest moves them to `raw/filings/<TICKER>/` (move-after-ingest convention; move only what was ingested, the rest stays as the queue). If Section 5 is empty, skip this step and say the local archive is already current. If a download fails (network/SEC rate limit), report which files succeeded and which failed — do not retry in a loop.

### 6. Rank the staged queue by thesis priority (added 2026-06-13)

End the run with a one-line-per-file **suggested ingest order** for everything now sitting in staging (not just this run's downloads). Rank by thesis centrality, judged from vault knowledge, highest first:

1. **Chokepoint-central names** (the ticker anchors or substantiates a chokepoint page — e.g., a laser-supply or CoWoS participant) and names with a **pre-registered trigger** the new filing could fire (check the page's Open questions)
2. **Theme-central / portfolio-anchor names** (multiple theme memberships, forward-edge-tracker subjects, hyperscaler-capex watch names)
3. **Periodic-cadence names** (vault coverage current, filing is routine)
4. **Peripheral / outside-framework names** (honest-verdict counterpoint pages — lowest urgency)

Format: `Suggested next refresh: 1. TICKER (form — one-phrase reason) 2. … `. One line of reasoning per file, no essays. This is a suggestion for Vic's sequencing decision, not an auto-queue — the skill never starts an ingest.
