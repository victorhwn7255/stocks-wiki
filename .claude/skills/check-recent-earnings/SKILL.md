...
- Non-SEC-registered foreign issuer (if added) — script tags as "out-of-scope; manual IR-page tracking required"
...
Always use this exact 3-section structure:
...
**Why these specific count caps:** 6 / 9 / 3 reflect Vic-curated operational display sizing — recent earnings and upcoming filings are the highest-value signals (more rows worth surfacing); stale tickers are tail-risk signals (3 rows sufficient to flag attention without overwhelming). The caps are display-only; the underlying scan covers all vault tickers.

**Why three sections:** Recent earnings = "ready to ingest now" (refresh ingest can proceed immediately with primary sources available); upcoming filings = "ingest queue" (refresh ingest sequencing for next 1-4 weeks); stale tickers = "refresh candidates" (operational hygiene check; tickers drifting out of vault canonical recency).

## Output discipline

- Concise output. Do not append explanatory paragraphs after the 3-section template unless the user explicitly asks for elaboration. The skill exists for fast operational scanning, not analytical commentary.
...
This skill maintains zero static state. SEC EDGAR is the canonical source for past filing dates; estimated next filings are derived from filing cadence (large accelerated filer windows: 10-Q within 40 days of quarter end; 10-K within 60 days of fiscal year end; 20-F within 4 months of fiscal year end). When new tickers are added to `scripts/fetch_filings.py` COMPANIES dict, they automatically appear in the next scan. No manual ticker list maintenance.
