...
description: Build (render) the Bloomberg-terminal HTML page for ONE company in the stocks-wiki vault from its wiki/companies/<TICKER>.md markdown, then run a deterministic QA audit and report pass/fail before opening it. Use this skill whenever the user wants to generate, build, render, rebuild, or refresh the website/HTML page for a specific company by ticker — e.g. "/build-company-html NVDA", "build the company page for NVDA", "render NVDA's webpage", "regenerate INTC's HTML page", "make the site page for SANHUA", "rebuild the HTML for TSM", or "I just ingested CRDO, build its web page". The skill is company-specialized (NOT for chokepoint / theme / tracker / thesis / framework pages). It only writes the gitignored build artifact under web/dist/ — it never edits wiki/ markdown, index.md, log.md, or any canon. Trigger on any request to produce or update the rendered HTML for a single named company; do not trigger for whole-vault site builds or for non-company page types.
---
...
- The user invokes `/build-company-html <TICKER>` or asks to build / render / rebuild / refresh the HTML or website page for a specific **company** by ticker.
- Right after a company's Markdown was ingested/edited and the user wants its rendered page updated.

Do **not** use this for: a whole-vault build (just run `python web/build.py`), a non-company page type (chokepoint/theme/tracker/thesis/framework — those have their own templates), or any request to change the company's Markdown/analysis (that's a human-gated ingest, not a render).

## Inputs

One ticker (the `wiki/companies/<TICKER>.md` slug, case-insensitive — upper-case it). Optional: `fast` to use the single-page build (`--only`) instead of the full site build (see Step 3).

## Step 1 — Preconditions

- Resolve `python3`.
- Confirm `web/build.py` exists (the generator + audit engine). If missing, stop and say the website generator isn't set up.
- Upper-case the ticker. Confirm `wiki/companies/<TICKER>.md` exists. If it does **not**, run the audit anyway (Step 2) to get the suggested near-matches, then STOP — report that there's no such company page and list the near-matches. **Never create the Markdown** (that's a canon/ingest action, not this skill).
...
- **Companies only** — if asked for a chokepoint/theme/etc., say it's out of scope (those get their own templates) and offer the whole-site build (`python web/build.py`) instead.
