...
**Two additional raw/ artifact folders (not skill-generated — awareness only, same non-canon / doesn't-count status).** (1) `raw/notes/twitter-notes/` — **Vic-staged** Tier-5 social (Twitter/X) exports; the manually-staged sibling of `video-intel/` (no skill writes to it). (2) `raw/research/charts/` — chart-**image** artifacts (PNG/JPG) that canon pages link to (e.g. the BofA semis/memory projection referenced by [[memory-shortage-winners-losers]] + [[HBM-oligopoly]]; the AI-credit-spread history used by the `AI-credit-spread-watch` tracker / `spread-watch` skill); the rest of `raw/research/` holds the Tier-3 deep-research anchors.
